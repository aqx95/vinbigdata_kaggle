...
class Csv2Coco:
    ...
    def to_coco(self, keys):
        ...
        for key in keys:
            ...
            for shape in shapes:
                bboxi = []
                for cor in shape[-7:-3]: # xmin,ymin,xmax,ymax
                    bboxi.append(int(float(cor)))
                label = shape[0] #label_id
...
if __name__ == '__main__':
    ...
    for fold in range(args.fold_num):
        ...
        l2c_train.save_coco_json(train_instance, os.path.join(annot_path,'instances_train.json'))
        ...
        l2c_val.save_coco_json(val_instance, os.path.join(annot_path, 'instances_val.json'))
